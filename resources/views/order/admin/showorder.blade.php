...
<div class="bg-gray-50 min-h-screen">
    <main class="w-full max-w-[1280px] mx-auto pt-24 px-4 sm:px-6 lg:px-8 pb-12 transition-all duration-300">
        ...
        <header class="mb-8 border-b border-[#e8dfd1] pb-6">
            <h1 class="font-['Literata'] text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900 mb-2">Kelola Semua Pesanan</h1>
            <p class="text-xs sm:text-sm text-gray-500 font-['Source_Serif_4'] italic">Pantau dan kelola seluruh transaksi yang terjadi di platform.</p>
        </header>
        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-[#e8dfd1] mb-8">
            <div class="flex flex-col sm:flex-row gap-4">
                <div class="flex-1 relative">
                    <input type="text" id="search-order" placeholder="Cari judul buku, penjual, atau pembeli..." 
        ...
        <div class="flex flex-col gap-6 sm:gap-8">
            @forelse ($orders as $order)
                <div class="order-card bg-white border border-[#e8dfd1] rounded-2xl overflow-hidden shadow-sm transition-all duration-300 hover:shadow-[0_8px_30px_rgb(122,79,55,0.06)]"
                    ...
                    data-seller="{{ strtolower($order->book->seller->name ?? '') }}"
                    data-buyer="{{ strtolower($order->buyer->name ?? '') }}">
                    ...
                    <div class="p-5 sm:p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 md:gap-8 border-b border-gray-100">
                        ...
                        <div class="flex-grow text-center sm:text-left">
                            ...
                            <div class="flex flex-wrap gap-3">
                                ...
                                <p class="text-xs text-gray-900 font-bold bg-gray-100 inline-block px-3 py-1 rounded-lg border border-gray-200/60">Penjual: <span class="text-[#5f3822]">{{ $order->book->seller->name ?? 'Tidak diketahui' }}</span></p>
                            </div>
                        </div>
                        ...
                        <div class="text-center sm:text-right flex flex-col items-center sm:items-end gap-1 shrink-0 bg-[#5f3822]/5 p-4 rounded-xl border border-[#5f3822]/5 w-full sm:w-auto">
                            <span class="text-[9px] font-bold tracking-wider text-gray-400 uppercase">Total Transaksi</span>
                    ...
                    <div class="px-5 py-4 sm:px-6 md:px-8 bg-gray-50/50 flex flex-col sm:flex-row justify-between items-center gap-3">
                        <p class="text-xs text-gray-900 font-semibold w-full sm:w-auto text-center sm:text-left">
                            Pembeli: <span class="text-[#5f3822] font-bold">{{ $order->buyer->name ?? 'Pelanggan' }}</span>
                        </p>
                        ...
                        <div class="flex flex-wrap items-center justify-center sm:justify-end gap-3 w-full sm:w-auto">
                            @if($order->status !== 'completed' && $order->status !== 'cancelled')
                                ...
                                <form action="{{ route('admin.orders.cancel', $order->id) }}" method="POST" class="w-full sm:w-auto" onsubmit="return confirm('Yakin ingin membatalkan pesanan ini?')">
                                    @csrf
                            ...
                            <a href="{{ route('admin.orders.detail', $order->id) }}" class="w-full sm:w-auto block text-center">
            ...
            @empty
                <div class="bg-white py-20 text-center rounded-2xl border border-[#e8dfd1] shadow-sm px-4">
                    ...
                    <p class="font-['Source_Serif_4'] text-gray-500 text-sm">Tidak ada pesanan yang terdeteksi.</p>
                </div>
            @endforelse
        </div>
        ...
        <div id="empty-order-alert" class="hidden bg-white py-20 text-center rounded-2xl border border-[#e8dfd1] shadow-sm px-4">
            ...
            <p class="font-['Source_Serif_4'] text-gray-500 text-sm">Tidak ditemukan pesanan yang cocok dengan kriteria filter Anda.</p>
        </div>
...
<div id="ratingModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 opacity-0 pointer-events-none transition-opacity duration-300">
    <div class="bg-white rounded-2xl max-w-md w-full p-6 sm:p-8 shadow-2xl border border-[#e8dfd1] transform scale-95 transition-transform duration-300 relative">
        ...
        <div class="mt-6 pt-4 border-t border-gray-100">
            <button type="button" onclick="closeRatingModal()" class="w-full bg-[#5f3822] text-white py-3 rounded-xl text-sm font-bold shadow-md hover:bg-[#7a4f37] transition-all">
                Selesai Membaca
            </button>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function filterOrders() {
            ...
            orderItems.forEach(item => {
                ...
                const buyer = item.getAttribute('data-buyer');

                const matchesKeyword = title.includes(keyword) || seller.includes(keyword) || buyer.includes(keyword);
