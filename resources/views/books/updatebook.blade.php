    ...
    <div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-10 text-center sm:text-left border-b border-[#e8dfd1] pb-6">
            ...
            <p class="text-xs sm:text-sm text-gray-500 mt-1 font-['Source_Serif_4'] italic">
                Silakan perbarui informasi bibliografi dan data inventaris buku Anda di bawah ini.  
            </p>
        </div>
                ...
                <div class="lg:col-span-8 space-y-6 sm:space-y-8">
                    ...
                    <section class="bg-white border border-[#e8dfd1] p-6 sm:p-8 rounded-2xl shadow-[0_4px_20px_-2px_rgba(122,79,55,0.03)] transition-all duration-300 hover:shadow-[0_6px_24px_-2px_rgba(122,79,55,0.05)]">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="title" value="{{ old('title', $book->title) }}" class="w-full bg-[#fcf9f0]/20 border border-gray-200 p-3 text-sm rounded-xl focus:outline-none focus:border-[#5f3822] focus:ring-1 focus:ring-[#5f3822] text-gray-900 shadow-sm transition-all" placeholder="Masukkan judul lengkap buku" required/>
                            ...
                            <div>
                                ...
                                <input type="text" name="author" value="{{ old('author', $book->author) }}" class="w-full bg-[#fcf9f0]/20 border border-gray-200 p-3 text-sm rounded-xl focus:outline-none focus:border-[#5f3822] focus:ring-1 focus:ring-[#5f3822] text-gray-900 shadow-sm transition-all" placeholder="Nama penulis atau kreator" required/>
                            ...
                            <div>
                                ...
                                <input type="text" name="category" value="{{ old('category', $book->category) }}" class="w-full bg-[#fcf9f0]/20 border border-gray-200 p-3 text-sm rounded-xl focus:outline-none focus:border-[#5f3822] focus:ring-1 focus:ring-[#5f3822] text-gray-900 shadow-sm transition-all" placeholder="Fiksi, Misteri, Sejarah" required/>
                                @error('category') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label class="block text-xs font-bold mb-2 text-gray-700 uppercase tracking-wider">Penerbit (Publisher)</label>
                                <input type="text" name="publisher" value="{{ old('publisher', $book->publisher) }}" class="w-full bg-[#fcf9f0]/20 border border-gray-200 p-3 text-sm rounded-xl focus:outline-none focus:border-[#5f3822] focus:ring-1 focus:ring-[#5f3822] text-gray-900 shadow-sm transition-all" placeholder="Nama perusahaan penerbit"/>
                            ...
                            <div class="md:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-5">
                                <div>
                                    ...
                                    <input type="number" name="year" min="1000" max="2100" value="{{ old('year', $book->year) }}" class="w-full bg-[#fcf9f0]/20 border border-gray-200 p-3 text-sm rounded-xl focus:outline-none focus:border-[#5f3822] focus:ring-1 focus:ring-[#5f3822] text-gray-900 shadow-sm transition-all" placeholder="Contoh: 2026" required/>
                                    @error('year') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                                </div>

                                <div>
                                    <label class="block text-xs font-bold mb-2 text-gray-700 uppercase tracking-wider">Halaman Buku</label>
                                    <input type="number" name="pages" min="1" value="{{ old('pages', $book->pages) }}" class="w-full bg-[#fcf9f0]/20 border border-gray-200 p-3 text-sm rounded-xl focus:outline-none focus:border-[#5f3822] focus:ring-1 focus:ring-[#5f3822] text-gray-900 shadow-sm transition-all" placeholder="Contoh: 350" required/>
                                    @error('pages') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="language" class="w-full bg-[#fcf9f0]/20 border border-gray-200 p-3 text-sm rounded-xl focus:outline-none focus:border-[#5f3822] focus:ring-1 focus:ring-[#5f3822] text-gray-900 shadow-sm transition-all">
                                        ...
                                        <option value="indonesia" {{ old('language', $book->language) == 'indonesia' ? 'selected' : '' }}>Bahasa Indonesia</option>
                                        <option value="french" {{ old('language', $book->language) == 'french' ? 'selected' : '' }}>French</option>
                                        <option value="latin" {{ old('language', $book->language) == 'latin' ? 'selected' : '' }}>Latin</option>
                    ...
                    <section class="bg-white border border-[#e8dfd1] p-6 sm:p-8 rounded-2xl shadow-[0_4px_20px_-2px_rgba(122,79,55,0.03)] transition-all duration-300 hover:shadow-[0_6px_24px_-2px_rgba(122,79,55,0.05)]">
                        ...
                        <div class="border border-gray-200 rounded-xl overflow-hidden focus-within:border-[#5f3822] focus-within:ring-1 focus-within:ring-[#5f3822] transition-all duration-200 shadow-sm">
                            <textarea name="description" class="w-full bg-[#fcf9f0]/10 border-none p-4 text-sm text-gray-900 placeholder-gray-400 resize-none focus:outline-none focus:ring-0" placeholder="Masukkan sinopsis atau deskripsi ringkas mengenai karya buku ini..." rows="7">{{ old('description', $book->description) }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                            <div>
                                ...
                                <div class="relative rounded-xl shadow-sm">
                                    ...
                                    <input type="number" name="price" value="{{ old('price', $book->price) }}" min="0" class="w-full bg-[#fcf9f0]/20 border border-gray-200 py-3 pl-10 pr-4 text-sm rounded-xl focus:outline-none focus:border-[#5f3822] focus:ring-1 focus:ring-[#5f3822] text-right text-gray-900 font-semibold transition-all" placeholder="185.000" required/>
                                </div>
                            ...
                            <div>
                                ...
                                <div class="relative rounded-xl shadow-sm">
                                    ...
                                    <input type="number" name="stock" value="{{ old('stock', $book->stock) }}" min="0" class="w-full bg-[#fcf9f0]/20 border border-gray-200 py-3 pl-10 pr-4 text-sm rounded-xl focus:outline-none focus:border-[#5f3822] focus:ring-1 focus:ring-[#5f3822] text-center text-gray-900 font-semibold transition-all" placeholder="12" required/>
                                </div>
                ...
                <div class="lg:col-span-4 space-y-6 sm:space-y-8">
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="flex flex-wrap gap-3.5 justify-start sm:justify-between items-center">
                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="cover_color" value="#5f3822" class="sr-only color-radio" {{ old('cover_color', $book->cover_color) == '#5f3822' ? 'checked' : '' }}>
                                    <span class="w-9 h-9 rounded-full block border border border-black/10 transition-all duration-200 shadow-sm group-hover:scale-105 ring-offset-2" style="background-color: #5f3822;"></span>
                                </label>

                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="cover_color" value="#3b5998" class="sr-only color-radio" {{ old('cover_color', $book->cover_color) == '#3b5998' ? 'checked' : '' }}>
                                    <span class="w-9 h-9 rounded-full block border border border-black/10 transition-all duration-200 shadow-sm group-hover:scale-105 ring-offset-2" style="background-color: #3b5998;"></span>
                                </label>

                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="cover_color" value="#e4405f" class="sr-only color-radio" {{ old('cover_color', $book->cover_color) == '#e4405f' ? 'checked' : '' }}>
                                    <span class="w-9 h-9 rounded-full block border border border-black/10 transition-all duration-200 shadow-sm group-hover:scale-105 ring-offset-2" style="background-color: #e4405f;"></span>
                                </label>

                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="cover_color" value="#55acee" class="sr-only color-radio" {{ old('cover_color', $book->cover_color) == '#55acee' ? 'checked' : '' }}>
                                    <span class="w-9 h-9 rounded-full block border border border-black/10 transition-all duration-200 shadow-sm group-hover:scale-105 ring-offset-2" style="background-color: #55acee;"></span>
                                </label>

                                <label class="relative cursor-pointer group">
                                    <input type="radio" name="cover_color" value="#0077b5" class="sr-only color-radio" {{ old('cover_color', $book->cover_color) == '#0077b5' ? 'checked' : '' }}>
                                    <span class="w-9 h-9 rounded-full block border border border-black/10 transition-all duration-200 shadow-sm group-hover:scale-105 ring-offset-2" style="background-color: #0077b5;"></span>
                                </label>
                            </div>
                            ...
                            <div class="pt-3 border-t border-gray-50">
                                ...
                                <div class="flex items-center gap-3">
                                    <div class="flex-1 relative rounded-xl shadow-sm">
                                        <input type="text" id="custom-color-text" name="cover_color_custom" value="{{ old('cover_color', $book->cover_color) }}" placeholder="#5f3822" maxlength="7" 
                                    ...
                                    <div id="custom-preview-box" class="w-10 h-10 rounded-xl border border-black/10 shadow-[inset_0_2px_4px_rgba(0,0,0,0.06)] flex items-center justify-center shrink-0 transition-all duration-300 relative overflow-hidden" style="background-color: {{ old('cover_color', $book->cover_color ?? '#5f3822') }};">
                    ...
                    <section class="bg-[#5f3822] border border-[#4d2d1b] p-6 sm:p-8 text-white shadow-xl flex flex-col gap-4 rounded-2xl relative overflow-hidden">
                        ...
                        <p class="text-xs sm:text-sm text-amber-50/80 font-medium leading-relaxed mb-3">
                            Silakan periksa kembali seluruh perubahan informasi spesifikasi buku sebelum menyimpannya ke sistem database.
                        </p>
