...
<main class="w-full pt-24 min-h-screen bg-[#fcf9f0] transition-all duration-300">
    <div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-wrap gap-3 mb-8">
            <div class="text-sm font-semibold text-gray-500 self-center mr-2">Laporan:</div>
            <a href="{{ route('seller.export.preview.sales') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-[#5f3822] text-white text-xs font-bold uppercase tracking-wider rounded-xl hover:bg-[#7a4f37] active:scale-[0.98] transition-all duration-200 shadow-sm">
                <span class="material-symbols-outlined text-sm">visibility</span> Preview Penjualan
            </a>
                                ...
                                <tr class="hover:bg-gray-50/60 transition-colors">
                                    ...
                                    <td class="py-4 text-center">
                                        @if($order->status === 'pending')
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider bg-yellow-50 text-yellow-700 border border-yellow-200/50">Pending</span>
                                        @elseif($order->status === 'processing')
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider bg-blue-50 text-blue-700 border border-blue-200/50">Diproses</span>
                                        @elseif($order->status === 'completed')
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider bg-green-50 text-green-700 border border-green-200/50">Selesai</span>
                                        @elseif($order->status === 'cancelled')
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider bg-red-50 text-red-700 border border-red-200/50">Batal</span>
                                        @else
                                            <span class="inline-flex items-center px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider bg-gray-50 text-gray-700 border border-gray-200/50">{{ $order->status }}</span>  
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
